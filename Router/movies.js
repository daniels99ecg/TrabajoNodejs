...
Router.get('/', async(req, res)=>{
    ...
    try {
        ...
        const result=await client.db('sample_mflix').collection('movies').find({}).toArray();
...
Router.put('/:id', async (req, res)=>{
    ...
    try {
        await client.connect();
        const result = await client.db("sample_mflix").collection("movies").updateMany({title:id},{$set:{title:body}});
        if(result){
            res.status(201).json({
                message: 'Se actualizo la pelicula',
                result,
...
Router.delete('/', async (req, res)=>{
   ...
   const body=req.body;
    ...
    try {
        await client.connect();
        const result = await client.db("sample_mflix").collection("movies").deleteMany(body);
        if(result){
            res.status(200).json({
                message: 'Se borro la pelicula',
                result,
